...
public class Bank extends SubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if(sender instanceof Player) {
            ...
            if(args.length < 1) {
                sender.sendMessage(Messages.withPrefix("error-bank-type", "&cMethod are requerd!"));
                return false;
            }
            ...
            if(args[0].equalsIgnoreCase("deposit")) {
                ...
                if(args.length < 2) {
                    sender.sendMessage(Messages.withPrefix("error-bank-amount", "&cAmount are requerd!"));
                    return false;
                }
            ...
            if(args[0].equalsIgnoreCase("balance")) {
                player.sendMessage(Messages.withPrefix("clan-bank-successful", "&eYou clan Balance: &a&l%balance%&e!")
    ...
    private boolean withdrawClan(Clan clan, double amount, Player player) {
        if (amount > clan.getBalance()) {
            player.sendMessage(Messages.withPrefix("clan-bank-no-amount-withdraw", "&cYour clan not have %amount%").replace("%amount%", economy.format(amount)));
            return true;
        }
    ...
    private boolean depositClan(Clan clan, double amount, Player player) {
            ...
            if (amount > economy.getBalance(player.getName())) {
                player.sendMessage(Messages.withPrefix("clan-bank-no-amount-deposit", "&cYou not have %amount%").replace("%amount%", economy.format(amount)));
                return true;
            }

            if(!clan.deposit(amount)){
                player.sendMessage(Messages.withPrefix("clan-bank-deposit-error", "&cInvalid deposit amount. Amount should be greater than 0."));
                return true;
            }
