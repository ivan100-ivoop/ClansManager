...
public class SetOptions extends SubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if(args.length < 1) {
            sender.sendMessage(Messages.withPrefix("missing-clan-option", "&cClan option is required!"));
            return false;
        }
            ...
            switch (args[0]){
                case "tp":
                    if(!this.clans.updateClanLocation(player)) {
                        player.sendMessage(Messages.withPrefix("clan-location-error", "&cUnsuccessful set new clan teleport location!"));
                        return true;
                    }

                    player.sendMessage(Messages.withPrefix("clan-location-updated", "&aSuccessful set new clan teleport location!"));
                    return true;
                case "owner":

                    if (args.length < 2) {
                        sender.sendMessage(Messages.withPrefix("missing-new-clan-owner", "&cClan new owner is required!"));
                        return true;
                    }
                    ...
                    if (newOwnerPlayer == null){
                        sender.sendMessage(Messages.withPrefix("player-offline", "&cThis player is offline!"));
                        return true;
                    }

                    if (clan.isOwner(newOwnerPlayer)) {
                        sender.sendMessage(Messages.withPrefix("error-clan-owner", "&cYou are unable to give the clan to yourself!"));
                        return true;
                    }
                    ...
                    sender.sendMessage(Messages.withPrefix("clan-owner-updated", "&aSuccessfully set new clan owner!"));
                    return true;
                case "prefix":
                    ...
                    if(clanPrefix.length() > this.maxPrefix){
                        sender.sendMessage(Messages.withPrefix("error-clan-prefix-count", "&cYour are unable to set to big prefix!"));
                        return true;
                    }

                    if(!this.clans.updateClanPrefix(player, clanPrefix)){
                        player.sendMessage(Messages.withPrefix("clan-prefix-error", "&cUnsuccessful set new clan prefix!"));
                        return true;
                    }

                    player.sendMessage(Messages.withPrefix("clan-prefix-updated", "&aSuccessful set new clan prefix!"));
                    return true;
                default:
                    player.sendMessage(Messages.withPrefix("clan-set-error", "&cOps, you can use only tp,prefix,owner"));
                    break;
            }
            return true;
        }
