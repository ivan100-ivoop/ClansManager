...
public class DBManager {
    ...
    public DBManager(Loader plugin) {
        defaultDBFile = new File(plugin.getDataFolder(), "clansmanager.db");
        prefix = plugin.getConfig().getString("db.prefix", "clansmanager_");
        host = plugin.getConfig().getString("db.host", "localhost");
        port = plugin.getConfig().getInt("db.port", 3306);
        database = plugin.getConfig().getString("db.name", "clansmanager");
    ...
    public String fixName(String table) {
        return prefix + table;
    }
