...
public class Arena {
    ...
    private static FileConfiguration getConfig(){
        File _arena = new File(Loader.instance.getDataFolder(), "arena.yml");
    ...
    public static void setInv(PlayerInventory inventory) {
        FileConfiguration arena = Arena.getConfig();
        arena.set("inv", inventory.getContents());
        Arena.saveConfig(arena);
    }

    public static ItemStack[] getInv() {
        FileConfiguration arena = Arena.getConfig();
        return arena.getList("inv").toArray(new ItemStack[0]);
    }
}
